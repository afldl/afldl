...
# 这段代码使用 collections.namedtuple() 函数创建了一个名为 Transform_1 的命名元组（named tuple）。
# 命名元组是一个具有命名字段的不可变数据结构，类似于普通的元组，但可以通过字段名访问元素，而不是通过索引。命名元组的字段名可以是字符串或数字，用于标识元组中的各个元素。
# 在这个例子中，Transform_1 的字段名依次为 'num'、'id' 和 'values'，分别表示一个数字、一个标识符和一个值列表。这样可以方便地通过字段名访问和操作元组中的数据，而不需要依赖于索引。
# 例如，可以通过 Transform_1.num、Transform_1.id 和 Transform_1.values 分别访问命名元组中的数字、标识符和值列表。
...
class Proposal_1:
    ...
    @classmethod
    def parse(cls, stream):  # 处理一个P载荷
        num, protocol, spi_size, n_transforms = struct.unpack('>BBBB', stream.read(4))  # protocol_ID = ISAKMP(1) 或 IPsec_ESP(3)
        spi = stream.read(spi_size)
        transforms = []
        more = True
        while more:  # 第一个循环处理T载荷
            more, length, tnum, id = struct.unpack('>BxHBB2x', stream.read(8))  # id = KEY_IKE(1) 或 AES(12)
            if protocol == enums.Protocol.ESP:
                ...
                values = attr_parse(stream, length-8, enums.TransformAttr)  # 解析获取这个T载荷的所有属性
                for attr_type in values:  # 第二个循环处理T载荷的属性
                    if attr_type in enums.TransformTable_1:
                        values[attr_type] = enums.TransformTable_1[attr_type](values[attr_type])
                transforms.append(Transform_1(tnum, enums.Protocol(id), values))  # 我猜num是属性的个数
    ...
    def to_bytes(self):
        ...
        for idx, transform in enumerate(self.transforms):  # 遍历的同时，返回下标和值
...
class PayloadCert_1(Payload):
    def __init__(self, cert_data):
        Payload.__init__(self, enums.Payload.CERT_1)  # enums.Payload.CERT_1=6
        self.cert_data = cert_data
    def parse_data(self, stream, length):
        self.cert_data = stream.read(length)
    def to_bytes(self):
        return self.cert_data
    def to_repr(self):
        return f'{self.cert_data.hex()}'

class PayloadSignature_1(Payload):
    def __init__(self, signature_data):
        Payload.__init__(self, enums.Payload.SIG_1)  # enums.Payload.SIG_1=9
        self.signature_data = signature_data
    def parse_data(self, stream, length):
        self.signature_data = stream.read(length)
    def to_bytes(self):
        return self.signature_data
    def to_repr(self):
        return f'{self.signature_data.hex()}'
...
class PayloadEAP(Payload):
    def __init__(self, code, data, critical=False):
        ...
        self.data = data
    def parse_data(self, stream, length):
        self.code = enums.EAPCode(struct.unpack('>B3x', stream.read(4))[0])
...
class PayloadCERT(Payload):
    def __init__(self, encoding, data, critical=False):
        Payload.__init__(self, enums.Payload.CERT, critical)
        self.encoding = enums.CertCode(encoding)
        self.data = data
    def parse_data(self, stream, length):
        self.encoding = enums.CertCode(struct.unpack('B', stream.read(1))[0])
        self.data = stream.read(length-1)
    def to_bytes(self):
        return struct.pack('B', self.encoding) + self.data
    def to_repr(self):
        return f'{self.encoding.name}({self.data.hex()})'

class PayloadCERTREQ(Payload):
    def __init__(self, encoding, data, critical=False):
        Payload.__init__(self, enums.Payload.CERTREQ, critical)
        self.encoding = enums.CertCode(encoding)
        self.data = data
    def parse_data(self, stream, length):
        self.encoding = enums.CertCode(struct.unpack('B', stream.read(1))[0])
...
PayloadClass = {
    ...
    enums.Payload.CERT: PayloadCERT,
    enums.Payload.CERTREQ: PayloadCERTREQ
}
...
class Message:
    ...
    def parse_payloads(self, stream, *, crypto=None):
        if crypto and self.flag & enums.MsgFlag.Encryption:
