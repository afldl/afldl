...
# replaces all outputs concatenated after KEX30+NEWKEY by the single output "BUFFERED", thus "KEX30+NEWKEYS+UA_SUCCESS*+UA_FAILURE*" becomes "KEX30+NEWKEYS+BUFFERED"
    ...
    def open_session(self, keyword=None, window_size=None, max_packet_size=None):
        """
    ...
    def open_channel(self,
        ...
        try:
            ...
            m = Message()
            if keyword == 'cMSG_CHANNEL_OPEN':
                print cMSG_CHANNEL_OPEN
                fuzz_value = os.urandom(len(cMSG_CHANNEL_OPEN))
                print 'fuzz_value'
                print fuzz_value
            ...
            if keyword == 'kind':
                print kind
                fuzz_value = os.urandom(len(kind))
                print 'fuzz_value'
    ...
    def fuzz_ssh_version(self, keyword=None):
        if keyword is not None:
            print "sent: client_protocol_version message."
            print "key_word:", keyword
            print "normal_value: "
            print self.local_version
            fuzz_value = os.urandom(20)
            print b"fuzz_value: "
            print fuzz_value
            self.packetizer.write_all(fuzz_value)
        else:
    ...
    def fuzz_kex_init(self, keyword=None):
        self._send_kex_init(keyword)
    ...
    def fuzz_kexdh_init(self, keyword=None):
        ...
        used_kex_engine.start_kex(keyword)
    ...
    def fuzz_disconnect(self):
        ...
        return self.read_multiple_responses(timeout=0.3, total_timeout=0.35)

    def fuzz_service_request_auth(self, keyword=None):
        self.auth_handler = AuthHandler(self)
        self.auth_handler._request_auth(keyword)
...
#this should be changed to the username on the server
        ...
        # self.auth_handler.username = data['username'] if ok else 'NOACCESS'
        self.auth_handler.username = self.username if ok else 'NOACCESS'
        print(self.auth_handler.username)
        print("-******************************************-")
        self.auth_handler.custom_parse_service_request()

        return self.read_multiple_responses(timeout=0.4, total_timeout=0.5)
...
#time params
    ...
    def fuzz_userauth_pw_ok(self, keyword=None):
        self.fuzz_userauth_pw(ok=True, keyword=keyword)
...
#set user and pwd
    def fuzz_userauth_pw(self, ok, keyword=None):
        self.auth_handler = AuthHandler(self)
        self.auth_handler.auth_method = 'password'
        # with open("username&password.json",'r') as load_f:
        #     data = json.load(load_f)
        self.auth_handler.username = self.username
        # print(self.auth_handler.username)
        # print(self.passwd)
        # print("-******************************************-")
        if ok:
            self.auth_handler.password = self.passwd  # Just an example, make sure this user/pass exists!
        else:
            self.auth_handler.password = 'ex@ampleThes2s'  # Just an example, make sure this user/pass is denied!
        self.auth_handler.custom_parse_service_request(keyword)
...
#time param
    ...
    def fuzz_newkeys(self, keyword=None):
        self._activate_outbound(keyword)

        return self.read_multiple_responses()

    def fuzz_channel_open(self, keyword=None):
        ...
        if len(self._channels):
            ...
            self.open_session(keyword=keyword)
            return self.read_multiple_responses()
    ...
    def fuzz_channel_close(self, keyword=None):
        try:
            # Send message
            m = Message()
            if keyword == 'cMSG_CHANNEL_CLOSE':
                # print("\\".join([i for i in list(cMSG_KEXINIT)]))
                print cMSG_CHANNEL_CLOSE
                fuzz_value = os.urandom(len(cMSG_CHANNEL_CLOSE))
                print 'fuzz_value'
                print fuzz_value
                # print("\\".join([i for i in list(fuzz_value)]))
                m.add_byte(fuzz_value)
            else:
    ...
    def _send_kex_init(self, keyword=None, first_follows=False):
        """
        ...
        m = Message()
        if keyword == 'cMSG_KEXINIT':
            # print("\\".join([i for i in list(cMSG_KEXINIT)]))
            print cMSG_KEXINIT
            fuzz_value = os.urandom(len(cMSG_KEXINIT))
            print 'fuzz_value'
            print fuzz_value
            # print("\\".join([i for i in list(fuzz_value)]))
            m.add_byte(fuzz_value)
        ...
        if keyword == '_preferred_kex':
            print self._preferred_kex
            fuzz_length = 0
            for value in self._preferred_kex:
                fuzz_length += len(value)
            fuzz_value = os.urandom(fuzz_length)
            print 'fuzz_value'
            print [i for i in list(fuzz_value)]
            # print("\\".join([i for i in list(fuzz_value)]))
            m.add_string(fuzz_value)
        ...
        if keyword == 'available_server_keys':
            print available_server_keys
            fuzz_length = 0
            for value in available_server_keys:
                fuzz_length += len(value)
            fuzz_value = os.urandom(fuzz_length)
            print 'fuzz_value'
            print [i for i in list(fuzz_value)]
        ...
        if keyword == '_preferred_ciphers':
            print self._preferred_ciphers
            fuzz_length = 0
            for value in self._preferred_ciphers:
                fuzz_length += len(value)
            fuzz_value = os.urandom(fuzz_length)
            print 'fuzz_value'
            print [i for i in list(fuzz_value)]
        ...
        if keyword == '_preferred_macs':
            print self._preferred_macs
            fuzz_length = 0
            for value in self._preferred_macs:
                fuzz_length += len(value)
            fuzz_value = os.urandom(fuzz_length)
            print 'fuzz_value'
            print [i for i in list(fuzz_value)]
        ...
        if keyword == '_preferred_compression':
            print self._preferred_compression
            fuzz_length = 0
            for value in self._preferred_compression:
                fuzz_length += len(value)
            fuzz_value = os.urandom(fuzz_length)
            print 'fuzz_value'
            print [i for i in list(fuzz_value)]
    ...
    def _activate_outbound(self, keyword=None):
        """switch on newly negotiated encryption parameters for outbound traffic"""
        m = Message()
        if keyword == 'cMSG_NEWKEYS':
            # print("\\".join([i for i in list(cMSG_KEXINIT)]))
            print cMSG_NEWKEYS
            fuzz_value = os.urandom(len(cMSG_NEWKEYS))
            print 'fuzz_value'
            print fuzz_value
            # print("\\".join([i for i in list(fuzz_value)]))
            m.add_byte(fuzz_value)
