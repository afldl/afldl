from TLSMapper.TLS10SUT import *
...
command = ['openssl', 's_server', '-cert', 'key/server.cer', '-key', 'key/server.key', '-Verify', '1', '-CAfile', 'key/ca.cer', '-keylogfile', 'key/key.log', '-HTTP']
...
sul = TLS10SUT(keyfile='./TLSMapper/key/declient.key', certfile='./TLSMapper/key/client.cer', ciphersuites=ciphersuites, target_cmd=command)
