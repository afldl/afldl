...
class TLS12SUT(LTLfSUT):
    ...
    def reset(self):
        ...
        sock.settimeout(0.5)
        if self.target_ip is None and self.target_port is None:
            sock.connect(('127.0.0.1',4433))
        else:
            sock.connect((self.target_ip,self.target_port))
        sock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
        self.TLS_client = TLSClient12(sock, ciphersuites=self.ciphersuites, privateKey=self.privateKey, cert_chain=self.cert_chain)
        # self.TLS_client.pre_set_extensions=[[(3,3)],[GroupName.x25519,GroupName.secp256r1],[(6,1),(5,1),(4,1),(6,3),(5,3),(4,3)]] # [versions,groups,sig_algs]
    ...
    def process_query(self, letter):
        ...
        if response in ['UnSupported', 'SigFailed', 'NoClientCert']:
            return 'NoResponse'
        return response
    ...
    def pre(self):
        ...
        if self.target_cmd:
            self.target_process = subprocess.Popen(self.target_cmd, shell=False, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            # self.target_process.stdin.write(b'111111\n')
