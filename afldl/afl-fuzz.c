...
u8 get_target_memory(){
  ...
  if(heap_info.size){
    ...
    if(get_heap_memory(heap_info,child_pid,buffer)){
      get_heap_data(buffer,heap_info.size,&memory_buf[messages_sent-1],memory_size);
      // save_png(&memory_buf[messages_sent-1],row,col,"tmp.png");
...
static void add_to_queue_with_state(u8* fname, u32 len, u8 passed_det, u32* states, size_t num_states) {
  ...
  for(int i = 0; i < q->region_count; ++i){
    ...
    // add 0 in state;
    int zero_state_id = 0;
    k = kh_get(hms, khms_states, zero_state_id);
    ...
    if (k != kh_end(khms_states)) {
      ...
      k = kh_put(hms, khms_states, zero_state_id, &discard);
      kh_value(khms_states, k) = newState;
    ...
    was_fuzzed_map[get_state_index(zero_state_id)][q->index] = 0; //Mark it as reachable but not fuzzed
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:j:J:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:P:KEq:s:RFc:l:")) > 0)
