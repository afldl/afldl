...
# Tutorial - Fuzzing openssl server with [AFLNet](https://github.com/aflnet/aflnet) and [AFLnwe](https://github.com/aflnet/aflnwe), a network-enabled version of AFL
Follow the steps below to run and collect experimental results for openssl, which is a lightweight File Transfer Protocol (FTP) server. The similar steps should be followed to run experiments on other subjects. Each subject program comes with a README.md file showing subject-specific commands to run experiments.

## Step-0. Set up environmental variables
```
git clone https://gitee.com/zhangph12138/profuzzbench-ju.git
cd profuzzbench
export PFBENCH=$(pwd)
...
The following commands create a docker image tagged openssl. The image should have everything available for fuzzing and code coverage collection.
...
```
## Step-1.5 test docker image is build properly

```bash
docker run -it openssl

cd aflml
./afl-fuzz -d -i /home/ubuntu/experiments/in-tls -x /home/ubuntu/experiments/tls.dict -o /home/ubuntu/experiments/out-openssl-aflml -N tcp://127.0.0.1/4433 -P TLS -D 10000 -q 3 -s 3 -E -K -R -W 100 -t 5000+ -m none /home/ubuntu/experiments/openssl/apps/openssl s_server -key /home/ubuntu/experiments/openssl/key.pem -cert /home/ubuntu/experiments/openssl/cert.pem -4 -naccept 1 -no_anti_replay
...
profuzzbench_exec_common.sh openssl 1 results-openssl aflnet out-openssl-aflnet "-P FTP -D 10000 -q 3 -s 3 -E -K" 3600 5 &
profuzzbench_exec_common.sh openssl 1 results-openssl aflnwe out-openssl-aflnwe "-D 10000 -K" 3600 5
profuzzbench_exec_common.sh openssl 1 results-openssl aflml out-openssl-aflml "-D 10000 -K" 3600 5
```
...
AFLNET: Collecting results and save them to results-openssl
...
1600905795,openssl,aflnwe,1,l_per,25.9
1600905795,openssl,aflnwe,1,l_abs,292
1600905795,openssl,aflnwe,1,b_per,13.3
1600905795,openssl,aflnwe,1,b_abs,108
1600905795,openssl,aflnwe,1,l_per,25.9
1600905795,openssl,aflnwe,1,l_abs,292
```
...
```
export MAKE_OPT="-j4"
docker build . -t openssl --build-arg MAKE_OPT
```
...
If you want to add a new protocol and/or a new target server (of a supported protocol), please follow the above folder structure and complete the steps below. We use openssl as an example.

### Step-1. Create a new folder containing the protocol/target server

The folder for openssl server is [subjects/FTP/openssl](subjects/FTP/openssl).
...
The following folder structure shows all files we have prepared for fuzzing openssl server. Please read [our paper](https://arxiv.org/pdf/2101.05102.pdf) to understand the purposes of these files.

```
subjects/FTP/openssl
...
Once a Docker image is successfully built, you should test your commands, as written in a target-specific [README.md](subjects/FTP/openssl/README.md), inside a single Docker container. For example, we run the following commands to check if everything is working for openssl.

```
//start a container
docker run -it openssl /bin/bash

//inside the docker container
//run a 60-min fuzzing experiment using AFLNet
cd experiments
run aflnet out-openssl-aflnet "-P FTP -D 10000 -q 3 -s 3 -E -K -c ./ftpclean.sh" 3600 5
```

If everything works, there should be no error messages and all the results are stored inside the out-openssl-aflnet folder.
